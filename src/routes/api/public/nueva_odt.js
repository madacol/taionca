    ...
    async (req, res) => {
        ...
        const {rows: odts} = await res.sql`
            INSERT INTO public.odts
                ( id_client, id_user, id_currency, id_entity, amount, description, id_quotation )
                VALUES (${id_client}::integer, ${user_id}::integer, ${id_currency}::integer, ${id_entity}::integer, ${amount}::numeric, ${description}::character varying, ${id_quotation}::integer)
                RETURNING id_odt;
        `;
